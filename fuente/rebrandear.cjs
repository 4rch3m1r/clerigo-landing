...
(function () {
  ...
  /* Y SIN ANIMAR MIENTRAS LA PÁGINA SE MUEVE. Un desplazamiento suave, aunque
     sea de la tira, CANCELA el que el navegador tenga en marcha en el
     documento. «Explorar la plataforma» salta once mil píxeles hacia abajo con
     scroll-behavior: smooth; si el pase de lámina caía a mitad de camino, la
     página se quedaba parada —o volvía arriba— y el botón parecía no hacer
     nada. Sólo funcionaba después de desplazarse a mano, porque entonces el
     carrusel ya no está a la vista y el pase se detiene. Medido: con la tira
     animada el salto acababa en 0; moviéndola de golpe, llegaba a 11.029.
     Así que mientras la página se desplaza —o acaban de pulsar un enlace a
     un ancla— la tira se recoloca sin animación. */
  var paginaEnMarcha = 0;
  window.addEventListener('scroll', function () { paginaEnMarcha = Date.now(); }, { passive: true });
  document.addEventListener('click', function (e) {
    if (e.target.closest && e.target.closest('a[href^="#"]')) paginaEnMarcha = Date.now();
  }, true);
  ...
  function centraLaTira() {
    ...
    if (Math.abs(franja.scrollLeft - destino) < 1) return;
    var suave = !pocoMovimiento && Date.now() - paginaEnMarcha > 1200;
    if (suave && franja.scrollTo) {
      franja.scrollTo({ left: destino, behavior: 'smooth' });
